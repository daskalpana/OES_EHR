...
r1 <- plogis(0.4) # P(S=1|Z=1)
r2 <- plogis(-0.6) # P(S=1|Z=0)
...
sel_model <- glm(S ~ Z, binomial(), data = valid_data) # logit(P(S=1|Z, D)) = c_0 + c_1 Z
sel_cov_matrix <- vcov(sel_model)

r1_est <- plogis(coef(summary(sel_model))[1] + coef(summary(sel_model))[2])
r2_est <- plogis(coef(summary(sel_model))[1])

grad_r1 <- r1_est* (1 - r1_est) * c(1, 1)
grad_r2 <- r2_est* (1 - r2_est) * c(1, 0)
...
samp_prob_eq <- function(var) {
  ...
  eq_3 <<- ((u_3 + u_4 + u_7 + u_8)/ var[1]) - (1 - plogis(theta_0_glm + theta_Z_glm))* p_z* nrow(valid_samp)/ p_s
  eq_4 <<- ((u_11 + u_12 + u_15 + u_16)/ var[2]) - (1 - plogis(theta_0_glm))* (1 - p_z)* nrow(valid_samp)/ p_s
  return(c(eq_1 + eq_3, eq_2 + eq_4))
}

samp_prob <- nleqslv(c(r1_est, r2_est), samp_prob_eq, jacobian = TRUE)
...
y_truth <- c(h1, h4, g1, g4, c1, c2, c3, c4, b1, b2, b3, b4, r1, r2)
y_est <- c(h1_est, h4_est, g1_est, g4_est, c1_est, c2_est, c3_est, c4_est, b1_est, b2_est, b3_est, b4_est, r1_est, r2_est)
y_est2 <- c(h1_est2, h4_est2, g1_est2, g4_est2, c1_est2, c2_est2, c3_est2, c4_est2, b1_est2, b2_est2, b3_est2, b4_est2, r1_est2, r2_est2)
y_joint <- c(r1_est, r2_est, theta_0_glm, theta_Z_glm)
...
two_step_mle <- function(x, y, data) {
  ...
  t_1 <<- (y[5] * y[1] * y[13] * plogis(x[1] + x[2]) * p_z + y[7] * (1 - y[2]) * y[14] * plogis(x[1]) * (1 - p_z) + (1 - y[9]) * y[3] * y[13] * (1 - plogis(x[1] + x[2])) * p_z + (1 - y[11]) * (1 - y[4]) * y[14] * (1 - plogis(x[1])) * (1 - p_z)) / 
    (y[13] * p_z + y[14] * (1 - p_z))
  
  t_2 <<- (y[6] * (1 - y[1]) * y[13] * plogis(x[1] + x[2]) * p_z + y[8] * y[2] * y[14] * plogis(x[1]) * (1 - p_z) + (1 - y[10]) * (1 - y[3]) * y[13] * (1 - plogis(x[1] + x[2])) * p_z + (1 - y[12]) * y[4] * y[14] * (1 - plogis(x[1])) * (1 - p_z)) / 
    (y[13] * p_z + y[14] * (1 - p_z))
  
  t_3 <<- ((1 - y[5]) * y[1] * y[13] * plogis(x[1] + x[2]) * p_z + (1 - y[7]) * (1 - y[2]) * y[14] * plogis(x[1]) * (1 - p_z) + y[9] * y[3] * y[13] * (1 - plogis(x[1] + x[2])) * p_z + y[11] * (1 - y[4]) * y[14] * (1 - plogis(x[1])) * (1 - p_z)) / 
    (y[13] * p_z + y[14] * (1 - p_z))
  
  t_4 <<- ((1 - y[6]) * (1 - y[1]) * y[13] * plogis(x[1] + x[2]) * p_z + (1 - y[8]) * y[2] * y[14] * plogis(x[1]) * (1 - p_z) + y[10] * (1 - y[3]) * y[13] * (1 - plogis(x[1] + x[2])) * p_z + y[12] * y[4] * y[14] * (1 - plogis(x[1])) * (1 - p_z)) / 
    (y[13] * p_z + y[14] * (1 - p_z))
  ...
  for (v in 1: nrow(data_samp)) {
    if (data_samp$ID[v] %in% valid_index) {
      ...
      p_2 <<- (1 - c1_est3)* h1_est3* x[1]* plogis(theta_0_glm + theta_Z_glm)* p_z/ 
        (x[1] * p_z + x[2] * (1 - p_z))
      p_3 <<- (1 - b1_est3)* g1_est3* x[1]* (1 - plogis(theta_0_glm + theta_Z_glm))* p_z/ 
        (x[1] * p_z + x[2] * (1 - p_z))
      p_4 <<- b1_est3* g1_est3* x[1]* (1 - plogis(theta_0_glm + theta_Z_glm))* p_z/ 
        (x[1] * p_z + x[2] * (1 - p_z))
      ...
      p_6 <<- (1 - c2_est3)* (1 - h1_est3)* x[1]* plogis(theta_0_glm + theta_Z_glm)* p_z/ 
        (x[1] * p_z + x[2] * (1 - p_z))
      p_7 <<- (1 - b2_est3)* (1 - g1_est3)* x[1]* (1 - plogis(theta_0_glm + theta_Z_glm))* p_z/ 
        (x[1] * p_z + x[2] * (1 - p_z))
      p_8 <<- b2_est3* (1 - g1_est3)* x[1]* (1 - plogis(theta_0_glm + theta_Z_glm))* p_z/ 
        (x[1] * p_z + x[2] * (1 - p_z))
      ...
      q_1 <<- (c1_est3 * h1_est3 * x[1] * plogis(x[3] + x[4]) * p_z + c3_est3 * (1 - h4_est3) * x[2] * plogis(x[3]) * (1 - p_z) + (1 - b1_est3) * g1_est3 * x[1] * (1 - plogis(x[3] + x[4])) * p_z + (1 - b3_est3) * (1 - g4_est3) * x[2] * (1 - plogis(x[3])) * (1 - p_z)) / 
        (x[1] * p_z + x[2] * (1 - p_z))
      
      q_2 <<- (c2_est3 * (1 - h1_est3) * x[1] * plogis(x[3] + x[4]) * p_z + c4_est3 * h4_est3 * x[2] * plogis(x[3]) * (1 - p_z) + (1 - b2_est3) * (1 - g1_est3) * x[1] * (1 - plogis(x[3] + x[4])) * p_z + (1 - b4_est3) * g4_est3 * x[2] * (1 - plogis(x[3])) * (1 - p_z)) / 
        (x[1] * p_z + x[2] * (1 - p_z))
      
      q_3 <<- ((1 - c1_est3) * h1_est3 * x[1] * plogis(x[3] + x[4]) * p_z + (1 - c3_est3) * (1 - h4_est3) * x[2] * plogis(x[3]) * (1 - p_z) + b1_est3 * g1_est3 * x[1] * (1 - plogis(x[3] + x[4])) * p_z + b3_est3 * (1 - g4_est3) * x[2] * (1 - plogis(x[3])) * (1 - p_z)) / 
        (x[1] * p_z + x[2] * (1 - p_z))
      
      q_4 <<- ((1 - c2_est3) * (1 - h1_est3) * x[1] * plogis(x[3] + x[4]) * p_z + (1 - c4_est3) * h4_est3 * x[2] * plogis(x[3]) * (1 - p_z) + b2_est3 * (1 - g1_est3) * x[1] * (1 - plogis(x[3] + x[4])) * p_z + b4_est3 * g4_est3 * x[2] * (1 - plogis(x[3])) * (1 - p_z)) / 
        (x[1] * p_z + x[2] * (1 - p_z))
...
write.csv(results, file = paste0("Outputs/Output1_case3/", i, ".csv"), row.names = FALSE)
write.csv(bias_qua, file = paste0("Outputs/Output2_case3/", i, ".csv"), row.names = FALSE)
write.csv(valid_stats, file = paste0("Outputs/Output3_case3/", i, ".csv"), row.names = FALSE)