...
sel_model <- glm(S ~ Z* D, binomial(), data = valid_data) # logit(P(S=1|Z, D)) = c_0 + c_1 Z + c_2 D + c_3 Z D
sel_cov_matrix <- vcov(sel_model)

r1_est <- plogis(coef(summary(sel_model))[1] + coef(summary(sel_model))[2] + coef(summary(sel_model))[3]  + coef(summary(sel_model))[4])
r2_est <- plogis(coef(summary(sel_model))[1] + coef(summary(sel_model))[3])
s1_est <- plogis(coef(summary(sel_model))[1] + coef(summary(sel_model))[2])
s2_est <- plogis(coef(summary(sel_model))[1])

grad_r1 <- r1_est* (1 - r1_est) * c(1, 1, 1, 1)
grad_r2 <- r2_est* (1 - r2_est) * c(1, 0, 1, 0)
grad_s1 <- s1_est* (1 - s1_est) * c(1, 1, 0, 0)
grad_s2 <- s2_est* (1 - s2_est) * c(1, 0, 0, 0)

r1_est_var <- (t(grad_r1) %*% sel_cov_matrix %*% grad_r1)
r2_est_var <- (t(grad_r2) %*% sel_cov_matrix %*% grad_r2)
s1_est_var <- (t(grad_s1) %*% sel_cov_matrix %*% grad_s1)
s2_est_var <- (t(grad_s2) %*% sel_cov_matrix %*% grad_s2)
...
c2_est2 <- ifelse(is.nan(c2_est2), c2_est, c2_est2)

c3_est2 <- valid_mle_est(u_9, u_10)
c3_est2 <- ifelse(is.nan(c3_est2), c3_est, c3_est2)
...
b2_est2 <- ifelse(is.nan(b2_est2), b2_est, b2_est2)

b3_est2 <- valid_mle_est(u_12, u_11)
b3_est2 <- ifelse(is.nan(b3_est2), b3_est, b3_est2)
...
samp_prob_eq <- function(var) {
  ...
  eq_2 <<- ((u_9 + u_10 + u_13 + u_14)/ var[2]) - plogis(theta_0_glm) * (1 - p_z)* nrow(valid_samp)/ p_s
  eq_3 <<- ((u_3 + u_4 + u_7 + u_8)/ var[3]) - (1 - plogis(theta_0_glm + theta_Z_glm))* p_z* nrow(valid_samp)/ p_s
  eq_4 <<- ((u_11 + u_12 + u_15 + u_16)/ var[4]) - (1 - plogis(theta_0_glm))* (1 - p_z)* nrow(valid_samp)/ p_s
  return(c(eq_1, eq_2, eq_3, eq_4))
}

samp_prob <- nleqslv(c(r1_est, r2_est, s1_est, s2_est), samp_prob_eq, jacobian = TRUE)
samp_prob_var <- diag(solve(t(samp_prob$jac) %*% samp_prob$jac))

r1_est2 <- samp_prob$x[1]
r2_est2 <- samp_prob$x[2]
s1_est2 <- samp_prob$x[3]
s2_est2 <- samp_prob$x[4]

r1_est2_var <- samp_prob_var[1]
r2_est2_var <- samp_prob_var[2]
s1_est2_var <- samp_prob_var[3]
s2_est2_var <- samp_prob_var[4]

# bias quantifier values for plug-in and two-step MLE estimates
y_truth <- c(h1, h4, g1, g4, c1, c2, c3, c4, b1, b2, b3, b4, r1, r2, s1, s2)
y_est <- c(h1_est, h4_est, g1_est, g4_est, c1_est, c2_est, c3_est, c4_est, b1_est, b2_est, b3_est, b4_est, r1_est, r2_est, s1_est, s2_est)
y_est2 <- c(h1_est2, h4_est2, g1_est2, g4_est2, c1_est2, c2_est2, c3_est2, c4_est2, b1_est2, b2_est2, b3_est2, b4_est2, r1_est2, r2_est2, s1_est2, s2_est2)
y_joint <- c(r1_est, r2_est, s1_est, s2_est, theta_0_glm, theta_Z_glm)
...
two_step_mle <- function(x, y, data) {
  ...
  t_1 <<- (y[5] * y[1] * y[13] * plogis(x[1] + x[2]) * p_z + y[7] * (1 - y[2]) * y[14] * plogis(x[1]) * (1 - p_z) + (1 - y[9]) * y[3] * y[15] * (1 - plogis(x[1] + x[2])) * p_z + (1 - y[11]) * (1 - y[4]) * y[16] * (1 - plogis(x[1])) * (1 - p_z)) / 
    (y[13] * plogis(x[1] + x[2]) * p_z + y[14] * plogis(x[1]) * (1 - p_z) + y[15] * (1 - plogis(x[1] + x[2])) * p_z + y[16] * (1 - plogis(x[1])) * (1 - p_z))
  
  t_2 <<- (y[6] * (1 - y[1]) * y[13] * plogis(x[1] + x[2]) * p_z + y[8] * y[2] * y[14] * plogis(x[1]) * (1 - p_z) + (1 - y[10]) * (1 - y[3]) * y[15] * (1 - plogis(x[1] + x[2])) * p_z + (1 - y[12]) * y[4] * y[16] * (1 - plogis(x[1])) * (1 - p_z)) / 
    (y[13] * plogis(x[1] + x[2]) * p_z + y[14] * plogis(x[1]) * (1 - p_z) + y[15] * (1 - plogis(x[1] + x[2])) * p_z + y[16] * (1 - plogis(x[1])) * (1 - p_z))
  
  t_3 <<- ((1 - y[5]) * y[1] * y[13] * plogis(x[1] + x[2]) * p_z + (1 - y[7]) * (1 - y[2]) * y[14] * plogis(x[1]) * (1 - p_z) + y[9] * y[3] * y[15] * (1 - plogis(x[1] + x[2])) * p_z + y[11] * (1 - y[4]) * y[16] * (1 - plogis(x[1])) * (1 - p_z)) / 
    (y[13] * plogis(x[1] + x[2]) * p_z + y[14] * plogis(x[1]) * (1 - p_z) + y[15] * (1 - plogis(x[1] + x[2])) * p_z + y[16] * (1 - plogis(x[1])) * (1 - p_z))
  
  t_4 <<- ((1 - y[6]) * (1 - y[1]) * y[13] * plogis(x[1] + x[2]) * p_z + (1 - y[8]) * y[2] * y[14] * plogis(x[1]) * (1 - p_z) + y[10] * (1 - y[3]) * y[15] * (1 - plogis(x[1] + x[2])) * p_z + y[12] * y[4] * y[16] * (1 - plogis(x[1])) * (1 - p_z)) / 
    (y[13] * plogis(x[1] + x[2]) * p_z + y[14] * plogis(x[1]) * (1 - p_z) + y[15] * (1 - plogis(x[1] + x[2])) * p_z + y[16] * (1 - plogis(x[1])) * (1 - p_z))
  ...
  for (v in 1: nrow(data_samp)) {
    if (data_samp$ID[v] %in% valid_index) {
      ...
      p_1 <<- c1_est3* h1_est3* x[1]* plogis(theta_0_glm + theta_Z_glm)* p_z/ 
        (x[1] * plogis(theta_0_glm + theta_Z_glm) * p_z + x[2] * plogis(theta_0_glm) * (1 - p_z) + x[3] * (1 - plogis(theta_0_glm + theta_Z_glm)) * p_z + x[4] * (1 - plogis(theta_0_glm)) * (1 - p_z))
      p_2 <<- (1 - c1_est3)* h1_est3* x[1]* plogis(theta_0_glm + theta_Z_glm)* p_z/ 
        (x[1] * plogis(theta_0_glm + theta_Z_glm) * p_z + x[2] * plogis(theta_0_glm) * (1 - p_z) + x[3] * (1 - plogis(theta_0_glm + theta_Z_glm)) * p_z + x[4] * (1 - plogis(theta_0_glm)) * (1 - p_z))
      p_3 <<- (1 - b1_est3)* g1_est3* x[3]* (1 - plogis(theta_0_glm + theta_Z_glm))* p_z/ 
        (x[1] * plogis(theta_0_glm + theta_Z_glm) * p_z + x[2] * plogis(theta_0_glm) * (1 - p_z) + x[3] * (1 - plogis(theta_0_glm + theta_Z_glm)) * p_z + x[4] * (1 - plogis(theta_0_glm)) * (1 - p_z))
      p_4 <<- b1_est3* g1_est3* x[3]* (1 - plogis(theta_0_glm + theta_Z_glm))* p_z/ 
        (x[1] * plogis(theta_0_glm + theta_Z_glm) * p_z + x[2] * plogis(theta_0_glm) * (1 - p_z) + x[3] * (1 - plogis(theta_0_glm + theta_Z_glm)) * p_z + x[4] * (1 - plogis(theta_0_glm)) * (1 - p_z))
      ...
      p_6 <<- (1 - c2_est3)* (1 - h1_est3)* x[1]* plogis(theta_0_glm + theta_Z_glm)* p_z/ 
        (x[1] * plogis(theta_0_glm + theta_Z_glm) * p_z + x[2] * plogis(theta_0_glm) * (1 - p_z) + x[3] * (1 - plogis(theta_0_glm + theta_Z_glm)) * p_z + x[4] * (1 - plogis(theta_0_glm)) * (1 - p_z))
      p_7 <<- (1 - b2_est3)* (1 - g1_est3)* x[3]* (1 - plogis(theta_0_glm + theta_Z_glm))* p_z/ 
        (x[1] * plogis(theta_0_glm + theta_Z_glm) * p_z + x[2] * plogis(theta_0_glm) * (1 - p_z) + x[3] * (1 - plogis(theta_0_glm + theta_Z_glm)) * p_z + x[4] * (1 - plogis(theta_0_glm)) * (1 - p_z))
      p_8 <<- b2_est3* (1 - g1_est3)* x[3]* (1 - plogis(theta_0_glm + theta_Z_glm))* p_z/ 
        (x[1] * plogis(theta_0_glm + theta_Z_glm) * p_z + x[2] * plogis(theta_0_glm) * (1 - p_z) + x[3] * (1 - plogis(theta_0_glm + theta_Z_glm)) * p_z + x[4] * (1 - plogis(theta_0_glm)) * (1 - p_z))
      
      p_9 <<- c3_est3* (1 - h4_est3)* x[2]* plogis(theta_0_glm)* (1 - p_z)/ 
        (x[1] * plogis(theta_0_glm + theta_Z_glm) * p_z + x[2] * plogis(theta_0_glm) * (1 - p_z) + x[3] * (1 - plogis(theta_0_glm + theta_Z_glm)) * p_z + x[4] * (1 - plogis(theta_0_glm)) * (1 - p_z))
      p_10 <<- (1 - c3_est3)* (1 - h4_est3)* x[2]* plogis(theta_0_glm)* (1 - p_z)/ 
        (x[1] * plogis(theta_0_glm + theta_Z_glm) * p_z + x[2] * plogis(theta_0_glm) * (1 - p_z) + x[3] * (1 - plogis(theta_0_glm + theta_Z_glm)) * p_z + x[4] * (1 - plogis(theta_0_glm)) * (1 - p_z))
      p_11 <<- (1 - b3_est3)* (1 - g4_est3)* x[4]* (1 - plogis(theta_0_glm))* (1 - p_z)/ 
        (x[1] * plogis(theta_0_glm + theta_Z_glm) * p_z + x[2] * plogis(theta_0_glm) * (1 - p_z) + x[3] * (1 - plogis(theta_0_glm + theta_Z_glm)) * p_z + x[4] * (1 - plogis(theta_0_glm)) * (1 - p_z))
      p_12 <<- b3_est3* (1 - g4_est3)* x[4]* (1 - plogis(theta_0_glm))* (1 - p_z)/ 
        (x[1] * plogis(theta_0_glm + theta_Z_glm) * p_z + x[2] * plogis(theta_0_glm) * (1 - p_z) + x[3] * (1 - plogis(theta_0_glm + theta_Z_glm)) * p_z + x[4] * (1 - plogis(theta_0_glm)) * (1 - p_z))
      
      p_13 <<- c4_est3* h4_est3* x[2]* plogis(theta_0_glm)* (1 - p_z)/ 
        (x[1] * plogis(theta_0_glm + theta_Z_glm) * p_z + x[2] * plogis(theta_0_glm) * (1 - p_z) + x[3] * (1 - plogis(theta_0_glm + theta_Z_glm)) * p_z + x[4] * (1 - plogis(theta_0_glm)) * (1 - p_z))
      p_14 <<- (1 - c4_est3)* h4_est3* x[2]* plogis(theta_0_glm)* (1 - p_z)/ 
        (x[1] * plogis(theta_0_glm + theta_Z_glm) * p_z + x[2] * plogis(theta_0_glm) * (1 - p_z) + x[3] * (1 - plogis(theta_0_glm + theta_Z_glm)) * p_z + x[4] * (1 - plogis(theta_0_glm)) * (1 - p_z))
      p_15 <<- (1 - b4_est3)* g4_est3* x[4]* (1 - plogis(theta_0_glm))* (1 - p_z)/ 
        (x[1] * plogis(theta_0_glm + theta_Z_glm) * p_z + x[2] * plogis(theta_0_glm) * (1 - p_z) + x[3] * (1 - plogis(theta_0_glm + theta_Z_glm)) * p_z + x[4] * (1 - plogis(theta_0_glm)) * (1 - p_z))
      p_16 <<- b4_est3* g4_est3* x[4]* (1 - plogis(theta_0_glm))* (1 - p_z)/ 
        (x[1] * plogis(theta_0_glm + theta_Z_glm) * p_z + x[2] * plogis(theta_0_glm) * (1 - p_z) + x[3] * (1 - plogis(theta_0_glm + theta_Z_glm)) * p_z + x[4] * (1 - plogis(theta_0_glm)) * (1 - p_z))
      ...
      q_1 <<- (c1_est3 * h1_est3 * x[1] * plogis(x[5] + x[6]) * p_z + c3_est3 * (1 - h4_est3) * x[2] * plogis(x[5]) * (1 - p_z) + (1 - b1_est3) * g1_est3 * x[3] * (1 - plogis(x[5] + x[6])) * p_z + (1 - b3_est3) * (1 - g4_est3) * x[4] * (1 - plogis(x[5])) * (1 - p_z)) / 
        (x[1] * plogis(x[5] + x[6]) * p_z + x[2] * plogis(x[5]) * (1 - p_z) + x[3] * (1 - plogis(x[5] + x[6])) * p_z + x[4] * (1 - plogis(x[5])) * (1 - p_z))
      
      q_2 <<- (c2_est3 * (1 - h1_est3) * x[1] * plogis(x[5] + x[6]) * p_z + c4_est3 * h4_est3 * x[2] * plogis(x[5]) * (1 - p_z) + (1 - b2_est3) * (1 - g1_est3) * x[3] * (1 - plogis(x[5] + x[6])) * p_z + (1 - b4_est3) * g4_est3 * x[4] * (1 - plogis(x[5])) * (1 - p_z)) / 
        (x[1] * plogis(x[5] + x[6]) * p_z + x[2] * plogis(x[5]) * (1 - p_z) + x[3] * (1 - plogis(x[5] + x[6])) * p_z + x[4] * (1 - plogis(x[5])) * (1 - p_z))
      
      q_3 <<- ((1 - c1_est3) * h1_est3 * x[1] * plogis(x[5] + x[6]) * p_z + (1 - c3_est3) * (1 - h4_est3) * x[2] * plogis(x[5]) * (1 - p_z) + b1_est3 * g1_est3 * x[3] * (1 - plogis(x[5] + x[6])) * p_z + b3_est3 * (1 - g4_est3) * x[4] * (1 - plogis(x[5])) * (1 - p_z)) / 
        (x[1] * plogis(x[5] + x[6]) * p_z + x[2] * plogis(x[5]) * (1 - p_z) + x[3] * (1 - plogis(x[5] + x[6])) * p_z + x[4] * (1 - plogis(x[5])) * (1 - p_z))
      
      q_4 <<- ((1 - c2_est3) * (1 - h1_est3) * x[1] * plogis(x[5] + x[6]) * p_z + (1 - c4_est3) * h4_est3 * x[2] * plogis(x[5]) * (1 - p_z) + b2_est3 * (1 - g1_est3) * x[3] * (1 - plogis(x[5] + x[6])) * p_z + b4_est3 * g4_est3 * x[4] * (1 - plogis(x[5])) * (1 - p_z)) / 
        (x[1] * plogis(x[5] + x[6]) * p_z + x[2] * plogis(x[5]) * (1 - p_z) + x[3] * (1 - plogis(x[5] + x[6])) * p_z + x[4] * (1 - plogis(x[5])) * (1 - p_z))
...
theta_0_mle4 <- mle4_result$par[5]
theta_Z_mle4 <- mle4_result$par[6]

mle4_hessian <- diag(solve(mle4_result$hessian))
theta_0_mle4_var <- mle4_hessian[5]
theta_Z_mle4_var <- mle4_hessian[6]

r1_est3 <- mle4_result$par[1]
r2_est3 <- mle4_result$par[2]
s1_est3 <- mle4_result$par[3]
s2_est3 <- mle4_result$par[4]

r1_est3_var <- mle4_hessian[1]
r2_est3_var <- mle4_hessian[2]
s1_est3_var <- mle4_hessian[3]
s2_est3_var <- mle4_hessian[4]
...
bias_qua <- bind_rows(
  ...
  data.frame(
    Parameter = "r2",
    Bias1 = round(mean(r2_est - r2), 4),
    Bias2 = round(mean(r2_est2 - r2), 4),
    Bias3 = round(mean(r2_est3 - r2), 4),
    Var1 = round(mean(r2_est_var), 4),
    Var2 = round(mean(r2_est2_var), 4),
    Var3 = round(mean(r2_est3_var), 4),
    Coverage1 = round(coverage(r2, r2_est, r2_est_var), 4),
    Coverage2 = round(coverage(r2, r2_est2, r2_est2_var), 4),
    Coverage3 = round(coverage(r2, r2_est3, r2_est3_var), 4)
  ),
  data.frame(
    Parameter = "s1",
  ...
  data.frame(
    Parameter = "s2",
    Bias1 = round(mean(s2_est - s2), 4),
    Bias2 = round(mean(s2_est2 - s2), 4),
    Bias3 = round(mean(s2_est3 - s2), 4),
    Var1 = round(mean(s2_est_var), 4),
    Var2 = round(mean(s2_est2_var), 4),
    Var3 = round(mean(s2_est3_var), 4),
    Coverage1 = round(coverage(s2, s2_est, s2_est_var), 4),
    Coverage2 = round(coverage(s2, s2_est2, s2_est2_var), 4),
    Coverage3 = round(coverage(s2, s2_est3, s2_est3_var), 4)
  )
)
...
write.csv(results, file = paste0("Outputs/Output1_case4/", i, ".csv"), row.names = FALSE)
write.csv(bias_qua, file = paste0("Outputs/Output2_case4/", i, ".csv"), row.names = FALSE)
write.csv(valid_stats, file = paste0("Outputs/Output3_case4/", i, ".csv"), row.names = FALSE)